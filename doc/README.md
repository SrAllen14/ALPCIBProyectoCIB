...
```bash
sudo useradd -d /var/home/html -s /bin/bash -g www-data operadorweb

# Un inconveniente bastante común es que no añada el usuario al grupo. La solución.
sudo usermod -aG www-data operadorweb2
...
```bash
# Primero deshabilitamos el mpm_prefork y nos saldrá un posible error.
